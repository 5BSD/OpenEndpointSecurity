...
main(void)
{
	...
	if (ioctl(fd, OES_IOC_CACHE_ADD, &entry) < 0) {
		perror("OES_IOC_CACHE_ADD allow");
		goto fail;
	}
	...
	key = entry.ece_key;
	if (ioctl(fd, OES_IOC_CACHE_REMOVE, &key) < 0) {
		perror("OES_IOC_CACHE_REMOVE");
		goto fail;
	}

/*
 * ESC GET_MODE / SET_TIMEOUT / GET_TIMEOUT API tests
 *
...
test_get_mode_initial(int fd)
{
	...
	if (ioctl(fd, ESC_IOC_GET_MODE, &args) < 0) {
		printf("FAIL (ioctl: %s)\n", strerror(errno));
		return (1);
	}

	/* Initial mode should be NOTIFY (0) */
	if (args.ema_mode != ESC_MODE_NOTIFY) {
		printf("FAIL (mode=%u, expected %u)\n",
		    args.ema_mode, ESC_MODE_NOTIFY);
		return (1);
	}

	/* Timeout should be default (30000ms) */
	if (args.ema_timeout_ms != ESC_DEFAULT_TIMEOUT_MS) {
		printf("FAIL (timeout=%u, expected %u)\n",
		    args.ema_timeout_ms, ESC_DEFAULT_TIMEOUT_MS);
		return (1);
	}
...
{
	struct esc_mode_args set_args, get_args;

	printf("  get_mode after set_mode: ");

	memset(&set_args, 0, sizeof(set_args));
	set_args.ema_mode = ESC_MODE_AUTH;
	set_args.ema_timeout_ms = 5000;
	set_args.ema_queue_size = 512;

	if (ioctl(fd, ESC_IOC_SET_MODE, &set_args) < 0) {
		printf("FAIL (set_mode: %s)\n", strerror(errno));
		return (1);
	}

	memset(&get_args, 0, sizeof(get_args));
	if (ioctl(fd, ESC_IOC_GET_MODE, &get_args) < 0) {
		printf("FAIL (get_mode: %s)\n", strerror(errno));
		return (1);
	}

	if (get_args.ema_mode != ESC_MODE_AUTH) {
		printf("FAIL (mode=%u, expected %u)\n",
		    get_args.ema_mode, ESC_MODE_AUTH);
		return (1);
	}
...
{
	struct esc_mode_args mode_args;
	struct esc_timeout_args timeout_args;

	printf("  set_timeout independent: ");

	/* First set mode with a specific timeout */
	memset(&mode_args, 0, sizeof(mode_args));
	mode_args.ema_mode = ESC_MODE_AUTH;
	mode_args.ema_timeout_ms = 10000;
	if (ioctl(fd, ESC_IOC_SET_MODE, &mode_args) < 0) {
		printf("FAIL (set_mode: %s)\n", strerror(errno));
		return (1);
	}
	...
	if (ioctl(fd, ESC_IOC_SET_TIMEOUT, &timeout_args) < 0) {
	...
	memset(&mode_args, 0, sizeof(mode_args));
	if (ioctl(fd, ESC_IOC_GET_MODE, &mode_args) < 0) {
		printf("FAIL (get_mode: %s)\n", strerror(errno));
		return (1);
	}

	if (mode_args.ema_mode != ESC_MODE_AUTH) {
...
{
	struct esc_timeout_args set_args, get_args;

	printf("  get_timeout: ");

	memset(&set_args, 0, sizeof(set_args));
	set_args.eta_timeout_ms = 15000;
	if (ioctl(fd, ESC_IOC_SET_TIMEOUT, &set_args) < 0) {
		printf("FAIL (set_timeout: %s)\n", strerror(errno));
		return (1);
	}

	memset(&get_args, 0, sizeof(get_args));
	if (ioctl(fd, ESC_IOC_GET_TIMEOUT, &get_args) < 0) {
...
test_timeout_clamping(int fd)
{
	...
	/* Test below minimum (should clamp to ESC_MIN_TIMEOUT_MS = 1000) */
	memset(&args, 0, sizeof(args));
	args.eta_timeout_ms = 100;
	if (ioctl(fd, ESC_IOC_SET_TIMEOUT, &args) < 0) {
		printf("FAIL (set_timeout min: %s)\n", strerror(errno));
		return (1);
	}

	memset(&args, 0, sizeof(args));
	if (ioctl(fd, ESC_IOC_GET_TIMEOUT, &args) < 0) {
		printf("FAIL (get_timeout: %s)\n", strerror(errno));
		return (1);
	}

	if (args.eta_timeout_ms != ESC_MIN_TIMEOUT_MS) {
		printf("FAIL (timeout=%u, expected min %u)\n",
		    args.eta_timeout_ms, ESC_MIN_TIMEOUT_MS);
		return (1);
	}

	/* Test above maximum (should clamp to ESC_MAX_TIMEOUT_MS = 300000) */
	memset(&args, 0, sizeof(args));
	...
	if (args.eta_timeout_ms != ESC_MAX_TIMEOUT_MS) {
		printf("FAIL (timeout=%u, expected max %u)\n",
		    args.eta_timeout_ms, ESC_MAX_TIMEOUT_MS);
		return (1);
	}
...
{
	struct esc_mode_args mode_args;
	struct esc_timeout_action_args action_args;
	...
	action_args.eta_action = ESC_AUTH_DENY;
	if (ioctl(fd, ESC_IOC_SET_TIMEOUT_ACTION, &action_args) < 0) {
	...
	if (stats.es_mode != ESC_MODE_AUTH) {
		printf("FAIL (stats.mode=%u, expected %u)\n",
		    stats.es_mode, ESC_MODE_AUTH);
		return (1);
	}
	...
	if (stats.es_timeout_action != ESC_AUTH_DENY) {
		printf("FAIL (stats.timeout_action=%u, expected %u)\n",
		    stats.es_timeout_action, ESC_AUTH_DENY);
		return (1);
	}
